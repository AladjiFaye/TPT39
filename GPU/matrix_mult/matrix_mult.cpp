...
//!! Malloc is useless if we use map
float *input_a;//=(float *) malloc(sizeof(float)*N);
//https://stackoverflow.com/questions/2128728/allocate-matrix-in-c
//for (unsigned i = 0; i < N; i++) input_a[i] = (float *) malloc(sizeof(float)*N);
float *input_b;//=(float *) malloc(sizeof(float)*N);
//for (unsigned i = 0; i < N; i++) input_b[i] = (float *) malloc(sizeof(float)*N);
float *output;//=(float *) malloc(sizeof(float)*N);
//for (unsigned i = 0; i < N; i++) output[i] = (float *) malloc(sizeof(float)*N);
float *ref_output=(float *) malloc(sizeof(float*)*N*N);
cl_mem input_a_buf; // num_devices elements
cl_mem input_b_buf; // num_devices elements
cl_mem output_buf; // num_devices elements

int number; //number of rows = number of columns
...
/*
		//CL_TRUE should be used for performance measurements instead of
		//CL_FALSE; CL_TRUE stops the whole process and waits for the end of
...
// Map to host memory
  ...
  number = N;
    ...
    for (unsigned k = 0; k < N; k++) {
				input_a[j+k*N] = rand_float();
				input_b[j+k*N] = rand_float();
			}
    }
    ...
    for (unsigned k = 0; k < N; k++) {
		    ref_output[j+k*N] = 0;
				for (unsigned i = 0; i < N; i++) {
					ref_output[j+k*N] += input_a[j+i*k]*input_b[i+k*N];
				}
    ...
    status = clSetKernelArg(kernel, argi++, sizeof(int), (void*)&number);
...
for(unsigned j = 0; j < N && pass; ++j) {
  for (unsigned k = 0; k < N && pass; k++) {
      if(fabsf(output[j+k*N] - ref_output[j+k*N]) > 1.0e-5f) {
        printf("Failed verification @ index %d\nOutput: %f\nReference: %f\n",
            j, output[j+k*N], ref_output[j+k*N]);
        pass = false;
        }
      }
}
